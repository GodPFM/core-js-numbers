...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const multiplier = x1 * x2 + y1 * y2;
  const absFirstVector = Math.sqrt(x1 ** 2 + y1 ** 2);
  const absSecondVector = Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(multiplier / (absFirstVector * absSecondVector));
}
...
function getSumOfDigits(num) {
  ...
  for (let i = String(num).length - 1; i >= 0; i -= 1) {
    const result = Math.trunc(number / 10 ** i);
    digits.push(result);
    number -= result * 10 ** i;
  }
  if (digits.length === 0) {
